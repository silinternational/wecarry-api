...
type sendGridTemplate struct {
	...
	// these fields will not be needed if the SendGrid-hosted templates work well
	subject, body string
}

var sendGridTemplateData = map[string]sendGridTemplate{
...
func (e *SendGridService) Send(msg Message) error {
	...
	templateData := sendGridTemplateData[msg.TemplateName]

	message := mail.NewV3Mail()

	message.SetTemplateID(templateData.id)
	message.SetFrom(mail.NewEmail(msg.FromName, msg.FromEmail))

	p := mail.NewPersonalization()
	to := []*mail.Email{
		mail.NewEmail(msg.ToName, msg.ToEmail),
	}
	p.AddTos(to...)
