...
func (ms *ModelSuite) TestSendNotificationRequestFromStatus() {
	...
	tests := []struct {
		...
		{name: "Good - Committed to Received",
			post:           posts[0],
			template:       domain.MessageTemplateRequestFromCommittedToReceived,
			sendFunction:   sendNotificationRequestFromCommittedOrAcceptedToReceived,
			wantEmailsSent: 1,
			wantToEmail:    posts[0].Provider.Email,
		},
		{name: "Bad - Committed to Received", // No Provider
			post:         posts[1],
			template:     domain.MessageTemplateRequestFromCommittedToReceived,
			sendFunction: sendNotificationRequestFromCommittedOrAcceptedToReceived,
			wantErrLog: fmt.Sprintf("error preparing '%s' notification - no provider\n",
				domain.MessageTemplateRequestFromCommittedToReceived),
		},
